...
import lcm
from mc_towr_data_t import mc_towr_data_t
...
def write_traj_to_file(time_traj, base_traj, jnt_traj, ee_pos_traj, ctact_traj):
    np.savetxt("time.csv", np.asarray(time_traj), delimiter=",", fmt='%8.4f')
    np.savetxt("body_state.csv", base_traj, delimiter=",", fmt='%8.4f')
    np.savetxt("ee_pos.csv", np.asarray(ee_pos_traj), delimiter=",", fmt='%8.4f')
    np.savetxt("jnt.csv", np.asarray(jnt_traj), delimiter=",", fmt='%8.4f')
    np.savetxt("contact.csv", np.asarray(ctact_traj), delimiter=",", fmt='%u')
